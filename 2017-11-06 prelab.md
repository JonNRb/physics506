...
We're analyzing AM waves, so the finite wavelength of light is important in
that respect. Also, some of our equipment is specialized for the red light our
laser uses since it relies on optical principles that depend on the wavelength.
Visible light is on the order of THz frequency and the AM waves we are
producing are on the MHz order, so we shouldn't have any issue with poor
sampling of the AM waves like we would if the wave frequency was closer to the
AM frequency.
